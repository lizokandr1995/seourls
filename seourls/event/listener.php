...
class listener implements EventSubscriberInterface
{
	...
	public function append_sid($event)
	{
		...
		if(str_contains($event['url'],'viewtopic.php'))
		{
			if(isset($event['params']["p"]))
			{
				$url="/post/".$event['params']["p"].'/';
				$event['url']=$url;
				$event['params']=null;
			}

			if(!is_array($event['params']) && str_starts_with($event['params'],"p=")  && !str_contains($event['params'],'hilit'))
			{
				$params=$event['params'];
				$matches=array();
				if(preg_match("/p=([^&#]+)(.*)/i",$params,$matches))
				{
					$url="/post/".$matches[1]."/".$matches[2];
					$event['url']=$url;
					$event['params']=null;
				}
			}
	...
	public function pagination_generate_page_link($event)
	{
		...
		if (!empty($this->topic_data) &&  isset($param['t'])) // && isset($param['f'])
		{
...
//		$viewtopic_url = $this->base->generate_topic_link($data['forum_id'], $data['forum_name'], $data['topic_id'], $data['topic_title'], $start) . '#p' . $event['row']['post_id'];
		...
		$viewtopic_url=$this->phpbb_root_path.'post/'.$event['row']['post_id'].'/'.'#p'.$event['row']['post_id'];
