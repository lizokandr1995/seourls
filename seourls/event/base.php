...
class base
{
	...
	{
		$seo_url="forum/$forum_id/topic/$topic_id/" . ($start ? 'start/' . $start : '');
		if ($full)
		{
			$seo_url= generate_board_url() . $seo_url;
		}
		else
		{
			if($this->phpbb_root_path=='./')
			{
				$seo_url= "/$seo_url";
			}
			else
			{
				$seo_url= $this->phpbb_root_path . $seo_url;
			}
		}
	...
	{
                $seo_url="forum/$forum_id/" .  ($start ? 'start/' . $start : '');
                if ($full)
                {
                        $seo_url= generate_board_url() . $seo_url;
                }
